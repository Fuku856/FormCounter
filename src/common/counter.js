...
//
// NEWLINE_SEG の + は CRLF のため。CRLF は 1 クラスタなので、改行 1 つ =
// 1 文字として数えられる。空白のほうは連続が 1 クラスタになることはない
// （UAX #29）ので、SPACE_SEG は 1 文字ぶんだけを見る。
//
// SPACE_SEG が \s 由来で自動的に含むもの:
...
FC.countCharacters = function (text, settings) {
  ...
  for (const { segment } of SEGMENTER.segment(text)) {
    ...
    // 1 符号単位なら、コードポイント数も絵文字判定も必ず 1 になる
    // （⌚ のような単体絵文字でもどちらの規則でも 1）。判定も配列生成も要らない。
    // 通常の日本語・英数はすべてここを通るので、この短絡が効く。
    if (segment.length === 1) {
      n += 1;
      continue;
    }
