...
// タグ付けと Release の作成は Actions の Release ワークフローが行う。
// ここで手動でタグを打つ必要はない（docs/RELEASE.md 参照）。
console.log(`OK  version ${current} -> ${next}`);
console.log(`    次: git commit -am "v${next}" && git push`);
console.log(`    そのあと Actions の Release を実行し、タグに v${next} を入力する`);
